...
setwd("D:/Work/general/R/data_science/3-r-data-science-m3-exercise-files/")

# Read a tab-delimited data file
cars <- read.table(
    file = "Cars.txt",
    header = TRUE,
    sep = "\t", #Seperated by tab in text file
    quote = "\"")
...
# Select a subset of columns #Uses dplyr
temp <- select(
    .data = cars, 
    Transmission,
    Cylinders,
...
# Chain methods together #Everything above but using piping to chain functions together.
efficiency <- cars %>%
