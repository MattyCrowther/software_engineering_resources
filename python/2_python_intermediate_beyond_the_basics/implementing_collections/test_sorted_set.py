'''
This is the test suite for the sorted set class in this directory. It is broken down into the seperate protocols
needed to build this class.
'''
import unittest
...
#Test class for container Protocol which tests __contains__
...
#Test class for sized protocol which tests __len__
...
#Test class for iterable protocol which tests __iter__
...
#Test class for sequence protocol which tests __getitem__ __add__ __mul__ __rmul__
#also index() and count() which are user functions. Finally __reversed__ which is standard (doesnt need implementation)
class TestSequenceProtocol(unittest.TestCase):
	...
	def test_repition_nonzero_right(self):
		s = SortedSet([4,5,6])
	...
	def test_repition_nonzero_left(self):
		s = SortedSet([4,5,6])
		self.assertEqual(100 * s ,s)
	

#Test class for repr protocol which tests __repr__
...
#Test class for value equality protocol which tests __eq__ __ne__
...
#Test class for set protocol which tests __le__ __lt__ __eq__ __ne__ __gt__ __ge__ __and__
# __or__ __xor__ __sub__ and isdisjoint()
