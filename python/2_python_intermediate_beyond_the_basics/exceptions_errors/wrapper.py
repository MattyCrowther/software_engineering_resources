...
def wrap(text, line_length):
    ...
    """
    if line_length < 1:
        raise ValueError("line_length {} is not positive".format(line_length)) #Could have thought to use assertions here but don't assertions are for developer bugs not user bugs.

    words = text.split()

    if max(map(len, words)) > line_length:
        raise ValueError("line_length must be at least as long as the longest word") #Could have thought to use assertions here but don't assertions are for developer bugs not user bugs.
    ...
    result = '\n'.join(lines)
    assert all(len(line) <= line_length for line in result.splitlines()) #Asserts all lines are a certain length assert the correctness of the code above not use input.
    return result
