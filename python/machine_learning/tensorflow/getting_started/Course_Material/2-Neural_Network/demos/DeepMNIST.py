#Please read SimpleMNIST.py as I haven't added comments which are in that one to this one.
...
x_image = tf.reshape(x, [-1,28,28,1], name="x_image") #[x,y,z] (-1 is flag for make this a list of the other dimensions  )
...
with tf.name_scope('Conv1'):
    W_conv1 = weight_variable([5, 5, 1, 32])#[sizex(5p),sizey(t),num of input chanel(1) bc greyscale if colour would be 3 (rgb),how many features determinted by each filter
...
# 2nd Convolution layer Gets bigger
...
with tf.name_scope('FullyConnected'):
    W_fc1 = weight_variable([7 * 7 * 64, 1024]) #7*7 features with 64 connections to 1024 neurons
...
# Readout layer
W_fc2 = weight_variable([1024, 10]) #[10=num of neurons 0-9]
