...
#Simply reads the data into memory.
...
with tf.name_scope("Input_Reshape"):
    #[x,y,z] (-1 is flag for make this a list of the other dimensions  )
    x_image = tf.reshape(x, [-1,28,28,1], name="x_image")
    tf.summary.image('input_img', x_image, 5) #exports current image to tensorboard
...
with tf.name_scope('Conv1'): #Added to help with naming, a context manager for naming instead of having to name each convalution layer seperatley. So will be Conv1/weight for exampl
    # 32 features for each 5X5 patch of the image
    with tf.name_scope('weights'):
        W_conv1 = weight_variable([5, 5, 1, 32], name="weight")#[sizex(5p),sizey(t),num of input chanel(1) bc greyscale if colour would be 3 (rgb),how many features determinted by each filter
    ...
    h_conv1 = tf.nn.relu(conv1_wx_b, name="relu")#Relu is rectified linear unit, an activation unit 
    tf.summary.histogram('h_conv1', h_conv1)
    # take results and run through max_pool
    h_pool1 = max_pool_2x2(h_conv1, name="pool")

# 2nd Convolution layer
with tf.name_scope('Conv2'): #Added to help with naming, a context manager for naming instead of having to name each convalution layer seperatley. 
...
with tf.name_scope('FC'): #Added to help with naming, a context manager for naming instead of having to name each convalution layer seperatley.
